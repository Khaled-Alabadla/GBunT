...
export default function ChatWindow({ messages, isLoading, streamingMessage, chatEndRef, messagesContainerRef, onSendMessage, t, language, onLanguageToggle }) {
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-[#1b120b] via-[#24180f] to-[#1b120b] relative overflow-hidden" style={{ maxHeight: '100vh' }}>
              ...
              <h1 className={`font-black pb-[5px] text-4xl tracking-widest drop-shadow-2xl ${isRTL ? 'font-tajawal' : 'font-poppins'}`}>
                {isRTL ? (
                  <>
                    ...
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-500 to-amber-500">بُن </span>
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-cream via-accent-light to-accent">تي</span>
                  </>
                ) : (
                  <>
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-cream via-accent-light to-accent">G</span>
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-yellow-500 to-amber-500">Bun</span>
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-cream via-accent-light to-accent">T</span>
                  </>
                )}
              </h1>
              <p className={`text-text/70 text-sm mt-1 font-medium tracking-wide ${isRTL ? 'font-cairo' : 'font-lato'}`}>
                {t.subtitle}
              </p>
            </div>
          </div>
      ...
      <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && (
          ...
          >
            ...
            >
              {isRTL ? 'متكلمنيش إلا عن القهوة بس يحب ☕' : 'I only talk about coffee. Do you have a coffee-related question? ☕'}
            </motion.h2>
        ...
        {messages.map((msg, i) => (
          <MessageBubble key={i} message={msg} isRTL={isRTL} language={language} />
        ))}
      ...
      >
        <div className={`w-full flex gap-2 px-4 ${isRTL ? 'flex-row' : 'flex-row-reverse'}`}>
          <motion.button
      ...
      <div className="bg-background/50 backdrop-blur-sm border-t border-accent/10 py-3 text-center">
        <p className="text-text/50 text-xs font-medium">
          Made with <span className="text-red-400">❤️</span> and <span className="text-accent">☕</span> by <span className="text-accent-light font-semibold">Khaled Alabadla</span>
        </p>
      </div>
    </div>
  );
